...
unit class App::Tasks:ver<0.4.0>:auth<zef:jmaslak>;
...
my multi sub trait_mod:<is>(Routine $r, :$locking) {
    $r.wrap: sub (|args) {
        my $self = args[0];
        $self.add-lock();
        ...
        $exception.rethrow with $exception;
        return $ret;
    };
}
...
# Indirectly tested
method get-next-sequence(-->Int) is locking {
...
# Has test
method task-new-expire-today(Str $sub?, App::Tasks::Task::Tag :$tag) is locking {
...
# Has test
method task-new-maturity(Str $sub?, Date:D :$maturity-date, App::Tasks::Task::Tag :$tag) is locking {
    my $task = self.task-new($sub, :$tag);
...
# Has test
method task-new(Str $sub?, App::Tasks::Task::Tag :$tag) is locking {
...
# Indirectly tested
method get-task-filename(Int $taskint where * ~~ ^100_000 --> IO::Path:D) is locking {
...
# Tested
method task-move(Int $old where * ~~ ^100_000, Int $new where * ~~ ^100_000) is locking {
...
method task-show(Int $tasknum where * ~~ ^100_000) is locking {
...
# Indirectly tested
method read-task-body(Hash $task is rw, @lines) is locking {
    for @lines -> $line {
        if $line ~~ m/^ '--- ' (\d+) $/ {
            my $bodydate = $0.Int;
            my $body = Hash.new;
...
# Tested
method task-retitle(
    Int $tasknum? where { !$tasknum.defined or $tasknum > 0 }, Str $newtitle? is copy
) is locking {
    if ! self.check-task-log {
...
# Indirectly tested
method retitle(Int $tasknum where * ~~ ^100_000, Str:D $newtitle) is locking {
    my $task = App::Tasks::Task.from-file($!tasks.data-dir, $tasknum);
...
# Tested
method task-set-expiration(Int $tasknum? is copy where { !$tasknum.defined or $tasknum > 0 }, Str $day? is copy) is locking {
    if $day.defined {
...
# Indirectly tested
method set-expiration(Int $tasknum where * ~~ ^100_000, Date:D $day) is locking {
    my $task = App::Tasks::Task.from-file($!tasks.data-dir, $tasknum);
...
# Tested
method task-set-maturity(Int $tasknum? is copy where { !$tasknum.defined or $tasknum > 0 }, Str $day? is copy) is locking {
    if $day.defined {
        if $day !~~ m/^ <[0..9]>**4 '-' <[0..9]><[0..9]> '-' <[0..9]><[0..9]> $/ {
            say "Invalid date format - please use YYYY-MM-DD format";
            return;
...
# Tested
method task-set-frequency(Int $tasknum? is copy where { !$tasknum.defined or $tasknum > 0 }, Int $frequency? is copy) is locking {
    if ! self.check-task-log() {
...
# Indirectly tested
method set-not-before(Int $tasknum where * ~~ ^100_000, Date:D $day) is locking {
    my $task = App::Tasks::Task.from-file($!tasks.data-dir, $tasknum);
...
method set-frequency(
    Int:D $tasknum where * ~~ ^100_000, Int:D $frequency where * ≥ 1
) is locking {
    my $task = App::Tasks::Task.from-file($!tasks.data-dir, $tasknum);
...
# Indirectly tested
method add-tag(
    Int:D $tasknum where * ~~ ^100_000, App::Tasks::Task::Tag:D $tag
) is locking {
    my $task = App::Tasks::Task.from-file($!tasks.data-dir, $tasknum);
...
# Tested
method task-add-note(Int $tasknum where * ~~ ^100_000, Str $orignote?) is locking {
    if ! self.check-task-log() {
...
# Indirectly tested
multi method add-note(Int:D $tasknum where * ~~ ^100_000, Str:D $note) is locking {
    my $task = App::Tasks::Task.from-file($!tasks.data-dir, $tasknum);
    return self.add-note($task, $note);
}

multi method add-note(App::Tasks::Task:D $task, Str:D $note) is locking {
    $task.add-note($note);
    $task.to-file;
...
method task-coalesce() is locking {
    self.coalesce-tasks();

    say "Coalesced tasks";
}

method coalesce-tasks() is locking {
...
method update-task-log() is locking {
...
method check-task-log() is locking {
...
method get-taskhash() is locking {
